...
public class DragUIItem : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    public void OnBeginDrag(PointerEventData data)
    {
        ...
        ShowObjectsWithTag("BlockedTowerInstance");
    }
    ...
    {
        Ray ray = new Ray(pos + Vector3.up * 10f, Vector3.down); // Tiro raycast desde arriba hacia abajo
        if (Physics.Raycast(ray, out RaycastHit hit, 20f))
        {
            ...
            if (tag == "Ground")
            {
                return true; // Solo permitimos sobre suelo
            }
            else
            {
                Debug.Log($"No puedes instanciar sobre: {tag}");
                return false; // Bloqueamos torres, enemigos y áreas bloqueadas
            }
        }

        // Si no tocó nada, no permitimos instanciar
        return false;
    }
